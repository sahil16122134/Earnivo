...
export async function qualifyReferral(env, completedUserId, timestamp) {
  ...
  const reward = config.reward; const writes = [writePatch(env, "users", user.id, { qualifiedTaskCount: nextQualifiedCount, referralStatus: "verified", referralVerifiedAt: timestamp, referralRewardCredited: true, referralRewardAmount: reward, updatedAt: timestamp }, { updateTime: user.updatedAt }), writeIncrement(env, "users", user.referredBy, { verifiedReferralCount: 1, referralRewardTotal: reward }, { updateTime: referrerProfile.updatedAt }), writeCreate(env, "notifications", `referral_qualified_${user.id}`, { userId: user.id, category: "referral", title: "Referral qualified", body: "You completed the required approved tasks for referral qualification.", unread: true, readAt: null, createdAt: timestamp }), writeCreate(env, "notifications", `referral_verified_${user.id}`, { userId: user.referredBy, category: "referral", title: reward > 0 ? "Referral reward credited" : "Referral verified", body: reward > 0 ? `A referral reward of ${formatINR(reward)} was credited to your wallet.` : "A referred member completed the qualifying action.", unread: true, readAt: null, createdAt: timestamp })];
  if (reward > 0) {
