...
import { referralFeedQuery, referralStatusCountQuery } from "./referral.js";
...
import { notificationFeedQuery, notificationRecipient, unreadNotificationCountQuery } from "./notifications.js";
import { assertWithdrawalPolicy, normalisePayoutMethod, normalisePayoutReference, withdrawalDailyCounterId, withdrawalDay, withdrawalPolicy } from "./withdrawal-policy.js";
...
async function withdrawals(request, env) {
  ...
  const method = normalisePayoutMethod(required(body.method, "Payout method")); const payoutReference = normalisePayoutReference(body.payoutReference); const timestamp = now(); const day = withdrawalDay(timestamp); const dailyId = withdrawalDailyCounterId(identity.uid, day); const [walletRecord, settings, dailyRecord] = await Promise.all([getDocument(env, "wallets", identity.uid), getDocument(env, "settings", "platform"), getDocument(env, "withdrawalDailyStats", dailyId)]);
  ...
  await commit(env, [
    ...
    writeCreate(env, "withdrawals", withdrawalId, { userId: identity.uid, amount, method, payoutReference, status: "pending", transactionId, createdAt: timestamp }),
...
function providerPayload(body) { return { name: required(cleanString(body.name, 120), "Provider name"), status: ["active", "disabled"].includes(body.status) ? body.status : "active", postbackNotes: cleanString(body.postbackNotes, 2000) }; }
function notificationPayload(body) { const category = cleanString(body.category || "general", 40).toLowerCase(); if (!["general", "task", "wallet", "withdrawal", "referral", "account"].includes(category)) throw new HttpError(400, "Notification category is invalid.", "invalid_notification_category"); return { title: required(cleanString(body.title, 120), "Notification title"), body: required(cleanString(body.body, 1000), "Notification message"), userId: notificationRecipient(body.userId), category, unread: true, readAt: null }; }
...
async function adminNotifications(request, env, identity, id) { if (id || request.method !== "POST") methodNotAllowed(); const payload = notificationPayload(await readJson(request)); const recipient = await getDocument(env, "users", payload.userId); if (!recipient) throw new HttpError(404, "The notification recipient was not found.", "notification_recipient_not_found"); const notificationId = crypto.randomUUID(); const result = await setDocument(env, "notifications", notificationId, { ...payload, createdAt: now(), createdBy: identity.uid }); await audit(env, identity.uid, "notification_sent", { id: notificationId, target: result.userId }); return result; }
