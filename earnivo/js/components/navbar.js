...
export async function mountNavbar(activeKey) {
  ...
  api.get("/v1/notifications?limit=1").then(({ unreadCount = 0 }) => { const badge = target.querySelector("[data-unread-badge]"); if (badge && unreadCount > 0) { badge.hidden = false; badge.textContent = unreadCount > 9 ? "9+" : String(unreadCount); } }).catch(() => {});
